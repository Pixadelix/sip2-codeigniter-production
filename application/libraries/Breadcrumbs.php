...
class Breadcrumbs {
	 ...
	 * @return	void
	 */		
	function push($page, $href, $icon = null)
	{
	 ...
	 * @return	void
	 */		
	function unshift($page, $href, $icon)
	{
		// no crumb provided
		if (!$page OR !$href) return;
		
		// Prepend site url
		$href = ci_site_url($href);
	...
	function show()
	{
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
					$output .= $this->crumb_last_open . ''.($crumb['icon'] ? '<i class="fa ' . $crumb['icon'] . '"></i>' : '') . ' ' . $crumb['page'] . '' . $this->crumb_close;
				} else {
					$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '">' . ($crumb['icon'] ? '<i class="fa ' . $crumb['icon'] . '"></i>' : '') . $crumb['page'] . '</a> '.$this->crumb_divider.$this->crumb_close;
				}
			}
