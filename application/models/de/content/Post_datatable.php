...
class Post_datatable extends Posts_datatable {
    ...
    protected function init_editor() {
		parent::init_editor();
		$this->post_cfg = $this->ci->config->item('post');
		$this->editor
            ->field( "$this->table.post_parent" )
                ...
                ->upload(
                    ...
                    ->where( function($q) { $q->where('post_type', Posts_datatable::TYPE_POST_ATTACHMENT, '='); } )
        ...
        $this->editor
            ->on( 'preCreate', function ( $editor, $values ) {
                $this->editor
                    ...
                    ->setValue(Posts_datatable::TYPE_POST);
            } )
	...
	private function _process_upload() {
		...
		if ( $this->ci->upload->do_upload('upload') ) {
			...
			$this->_attachment = [
				'post_type'            => Posts_datatable::TYPE_POST_ATTACHMENT,
	...
	public function get($id = null, $id2 = null) {
		...
		$this->editor
			->where('sip_posts.post_type', 'post', '=');
		parent::get($id, $id2);
